...
FastAPI service backing the IBM Bank core ledger. Provides OAuth2 token
issuance, account/customer lookup, transaction history, internal transfers,
and back-office operations (overdraft limits, manual fee reversals).

Run locally:
    uvicorn app.main:app --reload

Configuration (environment):
    COREBANK_DB_PATH   Path to the SQLite ledger (defaults to ../data/corebank.db)
"""
...
DB_PATH = os.getenv('COREBANK_DB_PATH', os.path.join(_BASE_DIR, 'data', 'corebank.db'))
oauth=OAuth2PasswordBearer(tokenUrl='token')
app=FastAPI(title='IBM Bank Core Banking API')

# Allow the teller web front-end (browser) to call the API across origins.
# Override the allowed origins in production via CORS_ALLOW_ORIGINS (comma-separated).
_origins = os.getenv('CORS_ALLOW_ORIGINS', '*').split(',')
app.add_middleware(
    CORSMiddleware,
    allow_origins=[o.strip() for o in _origins],
    allow_credentials=False,  # front-end authenticates with Bearer tokens, not cookies
    allow_methods=['*'],
    allow_headers=['*'],
)
