"""
IBM Bank Direct Operations Solution (prototype)

Natural-language router prototype that maps free-text teller/customer
requests to the IBM Bank CLI operations via direct command execution
(no agent framework), for predictable, reliable behaviour.

Prototype only — not part of the production services.
"""
...
# Resolve the sibling CLI services from the monorepo root.
CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
REPO_ROOT = os.path.abspath(os.path.join(CURRENT_DIR, "..", ".."))
PYTHON_PATH = sys.executable
TELLER_SCRIPT = os.path.join(REPO_ROOT, "services", "teller-cli", "teller_client.py")
BACKOFFICE_SCRIPT = os.path.join(REPO_ROOT, "services", "backoffice-cli", "backoffice_client.py")
...
class BankingSystem:
    ...
    def format_balance_response(self, balance_output):
        ...
        response = f"🏦 **IBM Bank Account Information** 🏦\n\n"
    ...
    def format_transfer_response(self, transfer_output, src_iban, dst_iban, amount):
        ...
        # Build nice formatted response
        response = f"🏦 **IBM Bank Transfer Service** 🏦\n\n"
    ...
    def format_overdraft_response(self, overdraft_output, iban, amount):
        ...
        # Build nice formatted response
        response = f"🏦 **IBM Bank Overdraft Service** 🏦\n\n"
    ...
    def format_reversal_response(self, reversal_output, iban, amount):
        ...
        # Build nice formatted response
        response = f"🏦 **IBM Bank Payment Reversal Service** 🏦\n\n"
...
def main():
    ...
    while True:
        ...
        if user_input.lower() == "exit":
            print("\n🏦 Thank you for banking with IBM Bank! Have a great day! 🏦")
            break
